...
void BIP324Cipher::Initialize(const EllSwiftPubKey& their_pubkey, bool initiator, bool self_decrypt, std::optional<MessageStartChars> network_magic) noexcept
{
    // Determine salt (fixed string + network magic bytes)
    const MessageStartChars& message_header = network_magic ? *network_magic : Params().MessageStart();
