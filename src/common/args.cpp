...
const char * const BITCOIN_CONF_FILENAME = "drivechain-ecash.conf";
...
fs::path GetDefaultDataDir()
{
    ...
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "drivechain-ecash";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "drivechain-ecash";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/drivechain-ecash";
#else
    // Unix-like
    return pathRet / ".drivechain-ecash";
#endif
#endif
}
